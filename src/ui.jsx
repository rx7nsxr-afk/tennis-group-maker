...
export function Button({ className = "", variant = "default", size, children, ...props }) {
  ...
  return (
    <button
      className={cx("inline-flex items-center justify-center gap-2 text-sm font-medium transition rounded-md", variantClass, sizeClass, className)}
      {...props}
    >
      {children}
    </button>
...
export function Input({ className = "", ...props }) {
  return <input className={cx("w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-slate-300", className)} {...props} />;
}
...
export function TabsTrigger({ value, className = "", children }) {
  ...
  return (
    <button
      ...
      className={cx("px-3 py-2 text-sm font-medium", active ? "bg-slate-900 text-white" : "text-slate-700 hover:bg-slate-100", className)}
    >
      {children}
    </button>
  );
